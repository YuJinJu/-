  ...
  if (board) {
    ...
    $.each(board, function (index, item) {
      ...
      <tr>
          ...
          <td >${item.title}</td> 
          <td >${item.contents}</td>
          <td > <button type="button" style="color:black; background-color:bisque;" class="btn" id="modifyBtn${index + 1
        }" data-toggle="modal" data-target="#modifyNotice${index + 1}">수정</button> </td>
          <td > <button type="button" style="color:white; background-color:gray;" class="btn" id="deleteBtn${index + 1
        }">삭제</button> </td>
